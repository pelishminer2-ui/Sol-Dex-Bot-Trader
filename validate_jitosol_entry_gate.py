"""Validate JitoSOL entry gates: positive day, $30 gain, +3.25% instant feasibility."""
...
def test_day_gate_requires_positive_day_and_usd_gain():
    assert jitosol_day_gate_passes(day_usd_gain=35.0, day_pct_gain=0.001) is True
    assert jitosol_day_gate_passes(day_usd_gain=35.0, day_pct_gain=0.0) is False
    assert jitosol_day_gate_passes(day_usd_gain=29.0, day_pct_gain=0.002) is False
    assert jitosol_day_gate_passes(day_usd_gain=None, day_pct_gain=0.01) is False
    print("PASS: day gate requires positive 24h and $30")


def test_day_gate_skip_reasons():
    assert "not positive" in jitosol_day_gate_skip_reason(
        day_usd_gain=35.0, day_pct_gain=0.0
    )
    assert "$29" in jitosol_day_gate_skip_reason(
        day_usd_gain=29.0, day_pct_gain=0.01
    )
    assert jitosol_day_gate_skip_reason(day_usd_gain=35.0, day_pct_gain=0.01) is None
...
def test_sol_entry_qualifies_uses_day_gate_for_jitosol():
    with patch.object(Config, "ENABLE_SOL_TRADING", True), patch.object(
        ...
        assert sol_entry_qualifies(None, day_usd_gain=35.0, day_pct_gain=0.01) is True
...
def test_strategy_blocks_jitosol_below_threshold():
    ...
    print("PASS: strategy blocks JitoSOL below $30 day gain")


def test_strategy_allows_jitosol_when_day_gate_passes():
    strategy = MomentumStrategy()
    eligible = _jitosol_candidate(day_usd_gain=35.0, day_pct_gain=0.001)
    with patch.object(Config, "ENABLE_SOL_TRADING", True), patch.object(
        Config, "SOL_TRADE_MINT", JITOSOL_MINT
    ):
        assert (
            strategy.evaluate_entry(
                eligible, 200.0, momentum=0.0, usd_gain=35.0
            )
            == SignalType.BUY
        )
